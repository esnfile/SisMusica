...
{
    public class BLL_ImportaPessoaErro : IBLL_ImportaPessoaErro
    {

        IDAL_ImportaPessoaItemErro objDAL;
        IBLL_buscaImportaPessoaErro objBLL;
            ...
            using (TransactionScope objTrans = new TransactionScope())
            {
                try
                {
                    ...
                    if (null != objEnt)
                    {
                        ...
                        //Chama a função que converte as datas
                        objEnt.Logs = new BLL_importaPessoaErro_Log().CriarLog(objEnt, "Update");
                        objEnt.Logs = new BLL_importaPessoaErro_Log().ValidaLog(objEnt.Logs);
                ...
                try
                {
                    ...
                    if (null != objEnt)
                    {
                        if ("0" != objEnt.CodImportaPessoaItem)
                        {
                            ...
                            //Chama a função que converte as datas
                            objEnt.Logs = new BLL_importaPessoaErro_Log().CriarLog(objEnt, "Delete");
                            objEnt.Logs = new BLL_importaPessoaErro_Log().ValidaLog(objEnt.Logs);

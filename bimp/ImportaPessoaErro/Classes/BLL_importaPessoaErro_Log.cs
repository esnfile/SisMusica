...
{
    public class BLL_importaPessoaErro_Log
    {
        /// <summary>
        ...
        /// <returns></returns>
        public MOD_log CriarLog(MOD_importaPessoaItemErro ent, string Operacao)
        {
            try
            {
                ...
                {
                    ent.Logs.CodRotina = Convert.ToString(new MOD_acessoImportaPessoaItemErro().RotEditImportaPessoaErro);
                    ent.Logs.Ocorrencia = "Foi Inserido o registro nº < " + ent.CodImportaPessoaItem + " > " +
                                         "gerado através da Importação nº < " + ent.CodImportaPessoa + " > " + "com erro do Irmão(ã): < " + ent.Nome + " > ";
                }

...
namespace ccbinst
{
    public partial class frmMtsModuloBusca : Form
    {
        public frmMtsModuloBusca(List<MOD_acessos> listaLibAcesso)
        {
            InitializeComponent();
            try
            {
        ...
        private void btnDescVisual_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ((frmMtsModulo)formulario).Text = "Visualizando Módulo de Estudo";
                ((frmMtsModulo)formulario).disabledForm();
            }
            catch (SqlException exl)
            {
                excp = new clsException(exl);
        ...
        private void btnDescIns_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                abrirForm(string.Empty, gridDesc);
                ((frmMtsModulo)formulario).Text = "Inserindo Módulo de Estudo";
                ((frmMtsModulo)formulario).enabledForm();
                ((frmMtsModulo)formulario).defineFoco();
            }
            catch (SqlException exl)
            {
                excp = new clsException(exl);
        ...
        private void btnDescEditar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Codigo = gridDesc[0, gridDesc.CurrentRow.Index].Value.ToString();
                abrirForm(string.Empty, gridDesc);
                ((frmMtsModulo)formulario).Text = "Editando Módulo de Estudo";
                ((frmMtsModulo)formulario).enabledForm();
                ((frmMtsModulo)formulario).defineFoco();
            }
            catch (SqlException exl)
        ...
        private void btnDescExc_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show(modulos.exclusao, "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    ...
                    funcoes.gridMtsModulo(gridDesc);
                }
                catch (SqlException exl)
                {
                    excp = new clsException(exl);
        ...
        private void frmMtsModuloBusca_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        internal void carregaGrid(string Pesquisa, string Campo, DataGridView dataGrid)
        {
            try
            {
                if (Pesquisa.Equals("Modulo"))
                {
                    ...
                    funcoes.gridMtsModulo(dataGrid);
        ...
        private void abrirForm(string form, DataGridView dataGrid)
        {
            ...
            {
                preencher(Codigo);
                formulario = new frmMtsModulo(this, dataGrid, lista);
                ((frmMtsModulo)formulario).MdiParent = MdiParent;
                ((frmMtsModulo)formulario).StartPosition = FormStartPosition.Manual;
                funcoes.CentralizarForm(((frmMtsModulo)formulario));
                ((frmMtsModulo)formulario).Show();
                ((frmMtsModulo)formulario).BringToFront();
                Enabled = false;
            }
            catch (SqlException exl)
            {
        ...
        internal void preencher(string CodModulo)
        {
            try
            {
                objBLL = new BLL_mtsModulo();
                lista = objBLL.buscarCod(CodModulo);
            }
            catch (SqlException exl)
            {
                throw exl;
        ...
        private MOD_mtsModulo criarTabela()
        {
            try
            {
                ...
                objEnt.CodModuloMts = Codigo;
                objEnt.DescModulo = Descricao;
        ...
        internal void verPermDesc(DataGridView dataGrid)
        {
            try
            {
                foreach (MOD_acessos ent in listaAcesso)
                {
                    ...
                    if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotInsMtsModulo))
                    {
                        btnDescIns.Enabled = true;
                    }
                    //verificando o botão editar
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotEditMtsModulo))
                    {
                    ...
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotExcMtsModulo))
                    {
                        if (dataGrid.Rows.Count > 0)
                        {
                    ...
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotVisMtsModulo))
                    {
                        if (dataGrid.Rows.Count > 0)
                        {

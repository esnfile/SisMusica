...
namespace ccbinst
{
    public partial class frmVozesBusca : Form
    {
        public frmVozesBusca(List<MOD_acessos> listaLibAcesso)
        {
            InitializeComponent();
            try
            {
        ...
        private void btnDescVisual_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ((frmVozes)formulario).Text = "Visualizando Vozes";
                ((frmVozes)formulario).disabledForm();
            }
            catch (SqlException exl)
            {
                excp = new clsException(exl);
        ...
        private void btnDescIns_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                abrirForm(string.Empty, gridDesc);
                ((frmVozes)formulario).Text = "Inserindo Vozes";
                ((frmVozes)formulario).enabledForm();
                ((frmVozes)formulario).defineFoco();
            }
            catch (SqlException exl)
            {
                excp = new clsException(exl);
        ...
        private void btnDescEditar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Codigo = gridDesc[0, gridDesc.CurrentRow.Index].Value.ToString();
                abrirForm(string.Empty, gridDesc);
                ((frmVozes)formulario).Text = "Editando Vozes";
                ((frmVozes)formulario).enabledForm();
                ((frmVozes)formulario).defineFoco();
            }
            catch (SqlException exl)
        ...
        private void frmVozesBusca_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        /// </summary>
        /// <param name="vCodVoz"></param>
        internal void preencher(string vCodVoz)
        {
            try
            {
                objBLL = new BLL_voz();
                lista = objBLL.buscarCod(vCodVoz);
            }
            catch (SqlException exl)
            {
                throw exl;
        ...
        private MOD_voz criarTabela()
        {
            try
            {
                ...
                objEnt = new MOD_voz();
                objEnt.CodVoz = Codigo;
                objEnt.DescVoz = Descricao;
        ...
        internal void verPermDesc(DataGridView dataGrid)
        {
            try
            {
                foreach (MOD_acessos ent in listaAcesso)
                {
                    ...
                    if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotInsVoz))
                    {
                        btnDescIns.Enabled = true;
                    }
                    //verificando o botão editar
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotEditVoz))
                    {
                    ...
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotExcVoz))
                    {
                        if (dataGrid.Rows.Count > 0)
                        {
                    ...
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotVisVoz))
                    {
                        if (dataGrid.Rows.Count > 0)
                        {

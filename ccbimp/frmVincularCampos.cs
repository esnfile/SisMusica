...
namespace ccbimp
{
    public partial class frmVincularCampos : Form
    {
        ...
        private void gridVincular_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                //habilitação das celulas após a entrada para edição
                if (gridVincular != null || gridVincular.Rows.Count > 0)
                {
                    if (gridVincular.Columns[e.ColumnIndex].Name.Equals("Limpa"))
                    {
                        //define algumas propridades no grid para atualização
                        gridVincular.ReadOnly = false;
                        //gridVincular.ModoOpera = DataGridViewPersonal.modoOpera.Edit;
                        gridVincular.Rows[gridVincular.CurrentRow.Index].Cells["CampoVinculado"].Value = null;
                        //gridVincular.Rows[gridVincular.CurrentRow.Index].Cells["CampoVinculado"].Selected = true;
                        //gridVincular.Columns[e.ColumnIndex].ReadOnly = false;
                        //gridVincular.BeginEdit(true);
                    }
        ...
        private void montaGridVincular()
        {
            try
            {
                ...
                colCampoImportado.HeaderCell.Style.ForeColor = Color.DarkSlateGray;
                colCampoImportado.DefaultCellStyle.ForeColor = Color.DarkSlateGray;
                ...
                colCampoVinculado.DisplayStyle = DataGridViewComboBoxDisplayStyle.Nothing;
                ...
                colDel.Width = 20;
                colDel.Frozen = false;
                colDel.MinimumWidth = 20;
                colDel.ReadOnly = true;
                colDel.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                colDel.AutoSizeMode = DataGridViewAutoSizeColumnMode.NotSet;
                colDel.Visible = true;
                colDel.Image = imgVincular.Images[0];
                colDel.ToolTipText = "Limpar o campo selecionado";
                ...
                gridVincular.Columns.Add(colDel);

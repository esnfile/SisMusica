...
namespace BLL.instrumentos
{
    public class BLL_familia
    {
        ...
        public bool inserir(MOD_familia objEnt)
        {
            using (TransactionScope objTrans = new TransactionScope())
            {
                try
                {
                    ...
                    objEnt.CodFamilia = Convert.ToString(this.retornaId());
        ...
        /// <returns></returns>
        public List<MOD_familia> buscarCod(string CodFamilia)
        {
            try
            {
                objDAL = new DAL_familia();
                objDtb = objDAL.buscarCod(CodFamilia);

                if (objDtb != null)
                {
                    listaFamilia = this.criarLista(objDtb);
                }
                return listaFamilia;
            }
            catch (SqlException exl)
        ...
        /// <returns></returns>
        public List<MOD_familia> buscarDescricao(string DescFamilia)
        {
            try
            {
                objDAL = new DAL_familia();
                objDtb = objDAL.buscarDescricao(DescFamilia + "%");
        ...
        /// <returns></returns>
        private List<MOD_familia> criarLista(DataTable objDtb)
        {
            try
            {
                //instancia a lista
        ...
        /// <returns></returns>
        private MOD_log criarLog(MOD_familia ent, string Operacao)
        {
            try
            {
                ...
                {
                    ent.Logs.CodRotina = Convert.ToString(modulos.rotInsFamilia);
                }
                else if (Operacao.Equals("Update"))
                {
                    ent.Logs.CodRotina = Convert.ToString(modulos.rotEditFamilia);
                }
                else if (Operacao.Equals("Delete"))
                {
                    ent.Logs.CodRotina = Convert.ToString(modulos.rotExcFamilia);
                }

                ent.Logs.NomePc = modulos.DescPc;
                ent.Logs.IpPc = modulos.IpPc;
                ent.Logs.Ocorrencia = "Código: < " + ent.CodFamilia + " > Descrição: < " + ent.DescFamilia + " > ";

...
namespace BLL.instrumentos
{
    public class BLL_mtsFase
    {
        ...
        List<MOD_mtsFase> listaMtsFase = new List<MOD_mtsFase>();
        ...
        public bool inserir(MOD_mtsFase objEnt)
        {
            using (TransactionScope objTrans = new TransactionScope())
            {
                try
                {
                    ...
                    objEnt.CodFaseMts = Convert.ToString(this.retornaId());
        ...
        /// <returns></returns>
        public List<MOD_mtsFase> buscarCod(string CodFaseMts)
        {
            try
            {
                objDAL = new DAL_mtsFase();
                objDtb = objDAL.buscarCod(CodFaseMts);

                if (objDtb != null)
                {
                    listaMtsFase = this.criarLista(objDtb);
                }
                return listaMtsFase;
            }
            catch (SqlException exl)
        ...
        /// <returns></returns>
        public List<MOD_mtsFase> buscarDescricao(string DescFase)
        {
            try
            {
                objDAL = new DAL_mtsFase();
                objDtb = objDAL.buscarDescricao(DescFase + "%");
        ...
        /// <returns></returns>
        private List<MOD_mtsFase> criarLista(DataTable objDtb)
        {
            try
            {
                //instancia a lista
                List<MOD_mtsFase> lista = new List<MOD_mtsFase>();
                //faz um loop no DataTable e preenche a lista
                foreach (DataRow row in objDtb.Rows)
                {
                    //instancia a entidade
                    MOD_mtsFase ent = new MOD_mtsFase();
        ...
        /// <returns></returns>
        private MOD_log criarLog(MOD_mtsFase ent, string Operacao)
        {
            try
            {
                ...
                {
                    ent.Logs.CodRotina = "75";
                }
                else if (Operacao.Equals("Update"))
                {
                    ent.Logs.CodRotina = "76";
                }
                else if (Operacao.Equals("Delete"))
                {
                    ent.Logs.CodRotina = "77";
                }

                ent.Logs.NomePc = modulos.DescPc;
                ent.Logs.IpPc = modulos.IpPc;
                ent.Logs.Ocorrencia = "Código: < " + ent.CodFaseMts + " > Descrição: < " + ent.DescFase + " > ";

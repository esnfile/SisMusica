...
namespace BLL.pessoa
{
    public class BLL_departamento
    {
        ...
        List<MOD_departamento> listaDepartamento = new List<MOD_departamento>();
        ...
        public bool salvar(MOD_departamento objEnt)
        {
            using (TransactionScope objTrans = new TransactionScope())
            {
                try
                {
                    ...
                    this.objDAL = new DAL_departamento();
        ...
        public bool inserir(MOD_departamento objEnt)
        {
            using (TransactionScope objTrans = new TransactionScope())
            {
                try
                {
                    ...
                    objEnt.CodDepartamento = Convert.ToString(this.retornaId());
        ...
        /// <returns></returns>
        public List<MOD_departamento> buscarCod(string CodDepartamento)
        {
            try
            {
                objDAL = new DAL_departamento();
                objDtb = objDAL.buscarCod(CodDepartamento);

                if (objDtb != null)
                {
                    listaDepartamento = this.criarLista(objDtb);
                }
        ...
        /// <returns></returns>
        public List<MOD_departamento> buscarDescricao(string DescDepartamento)
        {
            try
            {
                objDAL = new DAL_departamento();
                objDtb = objDAL.buscarDescricao(DescDepartamento + "%");
        ...
        /// <returns></returns>
        private List<MOD_departamento> criarLista(DataTable objDtb)
        {
            try
            {
                //instancia a lista
                List<MOD_departamento> lista = new List<MOD_departamento>();
                //faz um loop no DataTable e preenche a lista
                foreach (DataRow row in objDtb.Rows)
                {
                    //instancia a entidade
                    MOD_departamento ent = new MOD_departamento();
        ...
        /// <returns></returns>
        private MOD_log criarLog(MOD_departamento ent, string Operacao)
        {
            try
            {
                ...
                {
                    ent.Logs.CodRotina = Convert.ToString(MOD_acessoDepartamento.RotInsDepartamento);
                }
                else if (Operacao.Equals("Update"))
                {
                    ent.Logs.CodRotina = Convert.ToString(MOD_acessoDepartamento.RotEditDepartamento);
                }
                else if (Operacao.Equals("Delete"))
                {
                    ent.Logs.CodRotina = Convert.ToString(MOD_acessoDepartamento.RotExcDepartamento);
                }

                ent.Logs.NomePc = modulos.DescPc;
                ent.Logs.IpPc = modulos.IpPc;
                ent.Logs.Ocorrencia = "Código: < " + ent.CodDepartamento + " > Descrição: < " + ent.DescDepartamento + " > ";

    ...
    public partial class frmCCB : Form
    {
        ...
        internal void carregaRegiao(string vCodRegiao)
        {
            try
            {
                ...
                if (listaRegiao != null && listaRegiao.Count > 0)
                {
                    ...
                    lblDescricaoRegiao.Text = listaRegiao[0].Codigo + " - " + listaRegiao[0].Descricao;
        ...
        internal void verPermMinisterio()
        {
            try
            {
                ...
                {
                    //verificando a Aba Adicionais
                    if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotCCBAbaMinist))
                    {
                        tabMinisterio.Enabled = true;
        ...
        internal void verPermAlteraMinist()
        {
            try
            {
                foreach (MOD_acessos ent in listaAcesso)
                {
                    ...
                    if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotInsCCBMinist))
                    {
                        btnInserir.Enabled = true;
                    }
                    //verificando o botão visualizar
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotExcCCBMinist))
                    {

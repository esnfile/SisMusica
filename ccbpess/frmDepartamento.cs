...
namespace ccbpess
{
    ...
    {
        public frmDepartamento(Form forms, DataGridView gridPesquisa, List<MOD_departamento> lista)
        {
            InitializeComponent();
        ...
        List<MOD_departamento> listaDepartamento = null;
        ...
        private void frmDepartamento_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                if (Text.Equals("Visualizando Departamento"))
                {
                    e.Cancel = false;
                }
                else
        ...
        private void salvar()
        {
            try
            {
                if (ValidarControles(criarTabela()))
                {
                    ...
                    if (formChama.Name.Equals("frmDepartamentoBusca"))
                    {
                        ((frmDepartamentoBusca)formChama).carregaGrid("Departamento", objEnt.CodDepartamento, dataGrid);
                    }

                    FormClosing -= new FormClosingEventHandler(frmDepartamento_FormClosing);
        ...
        private MOD_departamento criarTabela()
        {
            try
            {
                ...
                objEnt.CodDepartamento = txtCodigo.Text;
                objEnt.DescDepartamento = txtDescricao.Text;
        ...
        /// </summary>
        /// <param name="listaDepartamento"></param>
        internal void preencher(List<MOD_departamento> listaDepartamento)
        {
            try
            {
                //informa os valores aos campos recebidos na lista
                txtCodigo.Text = listaDepartamento[0].CodDepartamento;
                txtDescricao.Text = listaDepartamento[0].DescDepartamento;
            }
            catch (SqlException exl)
            {
                throw exl;

...
namespace ccbpess
{
    public partial class frmPessoa : Form
    {
        ...
        BLL_metodoInstr objBLL_Metodo = null;
        ...
        private void tabAtende_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (tabAtende.SelectedTab.Name.Equals("tabAtendeRegiao"))
                {
                    disabledRegAtendPessoa();
                    if (listaRegiaoPessoa == null || listaRegiaoPessoa.Count < 1)
                    {
                        apoio.Aguarde();
                        carregaRegiaoPessoa(txtCodigo.Text);
                    }
                }
                else if (tabAtende.SelectedTab.Name.Equals("tabAtendeComum"))
                {
                    disabledAtendPessoa();
        ...
        #region Aba Atendimento Região

        private void txtRegPes_Enter(object sender, EventArgs e)
        {
            AcceptButton = btnCCBPesCod;
        }
        private void txtRegPes_Leave(object sender, EventArgs e)
        {
            if (!txtRegPes.Text.Trim().Equals(string.Empty))
            {
                try
                {
                    apoio.Aguarde();
                    carregaRegiao(txtRegPes.Text);
                }
                catch (SqlException exl)
                {
                    excp = new clsException(exl);
            ...
            {
                txtRegPes.Text = string.Empty;
                txtRegPesCod.Text = string.Empty;
                txtRegPesDesc.Text = string.Empty;
            }
        }
        private void btnRegPes_Click(object sender, EventArgs e)
        {
            try
            {
                apoio.Aguarde();
                abrirForm("frmReg", string.Empty);
            }
            catch (SqlException exl)
            {
                excp = new clsException(exl);
        ...
        private void btnRegPesInc_Click(object sender, EventArgs e)
        {
            try
            {
                apoio.AguardeGravar();
        ...
        private void btnRegPesRet_Click(object sender, EventArgs e)
        {
            try
            {
                limparRegAtendPessoa();
                disabledRegAtendPessoa();
            }
            catch (SqlException exl)
            {
        ...
        private void btnRegPesIns_Click(object sender, EventArgs e)
        {
            try
            {
                limparRegAtendPessoa();
                enabledRegAtendPessoa();
                gpoRegiaoPes.Enabled = true;
                txtRegPes.Focus();
            }
            catch (SqlException exl)
            {
        ...
        private void btnRegPesExc_Click(object sender, EventArgs e)
        {
            try
            {
                deleteRegiaoPessoa(gridRegiaoPes.CurrentRow.Index);
                disabledRegAtendPessoa();
            }
            catch (SqlException exl)
            {
        ...
        private void gridRegiaoPes_RowStateChanged(object sender, DataGridViewRowStateChangedEventArgs e)
        {
            try
            {
                //Verifica se o grid possui registro
                if (gridRegiaoPes.RowCount > 0)
                {
                    btnRegPesExc.Enabled = true;
                }
                else
                {
                    btnRegPesExc.Enabled = false;
                }
            }
            catch (SqlException exl)
            {
        ...
        private void gridRegiaoPes_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (gridRegiaoPes.RowCount > 0)
                {
                    IndiceRegiao = gridRegiaoPes.CurrentRow.Index;

                    if (IndiceRegiao > (-1))
                    {
                        CodRegiaoPessoa = gridRegiaoPes["CodRegiaoPessoa", IndiceRegiao].Value.ToString();
                    }
                }
            }
            catch (SqlException exl)
        ...
        private void gridCCBPes_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (gridCCBPes.RowCount > 0)
                {
                    ...
                    if (IndiceCCB > (-1))
                    {
                        CodCCBPessoa = gridCCBPes["CodCCBPessoa", IndiceCCB].Value.ToString();
                    }
                }
            }
            catch (SqlException exl)
        ...
        private void tabPessoa_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //Preenche as regioes caso tenha
                    if (tabAtendeRegiao.Enabled.Equals(true))
                    {
                        if (listaRegiaoPessoa == null || listaRegiaoPessoa.Count < 1)
                        {
                            apoio.Aguarde();
                    ...
                    //Preenche as comuns caso tenha
                    else if (tabAtendeComum.Enabled.Equals(true))
                    {
                        if (listaCCBPessoa == null || listaCCBPessoa.Count < 1)
                        {
                            apoio.Aguarde();
            ...
            try
            {
                if (ValidarControles(criarTabela()))
                {
                    ...
                    if (Convert.ToInt64(txtCodigo.Text).Equals(0))
                    {
                        ...
                        blnRetorno = objBLL.inserir(criarTabela());
                    }
                    else
                    {
                        //chama a rotina da camada de negocios para efetuar inserção ou update
                        blnRetorno = objBLL.salvar(criarTabela());
                    }

                    //conversor para retorno ao formulario que chamou
                    if (formChama.Name.Equals("frmPessoaBusca"))
                    {
                        ((frmPessoaBusca)formChama).carregaGrid("Pessoa", objEnt.CodPessoa, dataGrid);
                    }
        ...
        private MOD_pessoa criarTabela()
        {
            try
            {
                ...
                objEnt.PermiteInstrumento = CargoSelecao.PermiteInstrumento;
                ...
                {
                    objEnt.listaCCBPessoa = new BindingList<MOD_ccbPessoa>();
                    objEnt.listaDeleteCCBPessoa = new List<MOD_ccbPessoa>();
        ...
        internal void carregaRegiao(string vCodRegiao)
        {
            try
            {
                ...
                {
                    txtRegPes.Text = listaRegiao[0].CodRegiao.PadLeft(3, '0');
                    txtRegPesCod.Text = listaRegiao[0].Codigo;
                    txtRegPesDesc.Text = listaRegiao[0].DescRegiao;
                }
                else
                {
                    abrirForm("frmRegiao", string.Empty);
                }
            }
            catch (SqlException exl)
            {
        ...
        internal void disabledForm()
        {
            try
            {
                ...
                gridRegiaoPes.ColumnHeadersDefaultCellStyle.ForeColor = Color.Gray;
        ...
        private void carregaCCBPessoa(string CodPessoa)
        {
            try
            {
                ...
                listaCCBPessoa = objBLL.buscarCCBPessoa(CodPessoa);
        ...
        private void inserirCCBPessoa()
        {
            try
            {
                ...
                ent.Codigo = txtCCBPesIdent.Text;
                ent.Descricao = txtCCBPesDesc.Text;
        ...
        #region Grid RegiaoPessoa

        /// <summary>
        /// Função que carrega as Regiões da Pessoa
        /// </summary>
        /// <param name="CodPessoa"></param>
        private void carregaRegiaoPessoa(string CodPessoa)
        {
            try
            {
                objBLL = new BLL_pessoa();
                objBinding_Regiao = new BindingSource();
                listaRegiaoPessoa = objBLL.buscarRegiaoPessoa(CodPessoa);

                objBinding_Regiao.DataSource = listaRegiaoPessoa;


                funcoes.gridRegiao(gridRegiaoPes, "RegiaoPessoa");
                ///vincula a lista ao DataSource do DataGriView
                gridRegiaoPes.DataSource = objBinding_Regiao;
            }
            catch (SqlException exl)
            {
                throw exl;
        ...
        /// Função que insere uma nova linha no DataGridView
        /// </summary>
        private void inserirRegiaoPessoa()
        {
            try
            {
                MOD_regiaoPessoa ent = new MOD_regiaoPessoa();
                ent.CodRegiaoPessoa = "0";
                ent.CodRegiao = txtRegPes.Text;
                ent.Codigo = txtRegPesCod.Text;
                ent.DescRegiao = txtRegPesDesc.Text;

                listaRegiaoPessoa = ((BindingList<MOD_regiaoPessoa>)objBinding_Regiao.DataSource);
                //adiciona um novo item a lista
                listaRegiaoPessoa.Add(ent);
                //atualiza o datagridview
                listaRegiaoPessoa.ResetItem(gridRegiaoPes.RowCount - 1);
         ...
         /// <summary>
        /// Função que Deleta a linha selecionada no DataGridView
        /// </summary>
        private void deleteRegiaoPessoa(int Indice)
        {
            try
            {
                MOD_regiaoPessoa ent = new MOD_regiaoPessoa();
                ent.CodRegiaoPessoa = CodRegiaoPessoa;

                //Insere a linha na Lista Delete
                listaDeleteRegiaoPessoa.Add(ent);
                //Exclui a linha da lista
                listaRegiaoPessoa.RemoveAt(Indice);

                //Seleciona a linha anterior a excluida
                if (gridRegiaoPes.RowCount > 0)
                {
                    if (!gridRegiaoPes.Rows[0].Selected.Equals(true))
                    {
                        gridRegiaoPes.Rows[Indice - 1].Selected = true;
                    }
                    else
                    {
                        gridRegiaoPes.Rows[gridRegiaoPes.RowCount - 1].Selected = true;
                    }
                }
            }
            catch (SqlException exl)
        ...
        /// Função que limpa os controle para Inserir nova CCBPessoa
        /// </summary>
        private void limparRegAtendPessoa()
        {
            try
            {
                txtRegPes.Text = string.Empty;
        ...
        /// Função que deabilita os controles
        /// </summary>
        internal void disabledRegAtendPessoa()
        {
            try
            {
                gpoRegiaoPes.Enabled = false;
                gridRegiaoPes.Enabled = true;
                btnRegPesIns.Enabled = true;
        ...
        /// Função que habilita os controles
        /// </summary>
        internal void enabledRegAtendPessoa()
        {
            try
            {
                gpoRegiaoPes.Enabled = true;
                gridRegiaoPes.Enabled = false;
                btnRegPesIns.Enabled = false;
                btnRegPesExc.Enabled = false;
            }
            catch (SqlException exl)
            {
        ...
        private void carregaInstrumentos(string CodPessoa)
        {
            try
            {
                ...
                objBLL = new BLL_pessoa();
                listaPessoaInstr = objBLL.buscarInstPessoa(CodPessoa);

                objBLL = new BLL_pessoa();
                listaInstrumento = objBLL.buscarInstrumentos(CodPessoa);
                ...
                foreach (MOD_instrumento objEnt_Inst in listaInstrumento)
                {
                    ...
                    ent.DescFamilia = objEnt_Inst.DescFamilia;
                    ent.CodPessoa = "0";
                    ent.Marcado = false;
        ...
        private void carregaMetodos(string CodPessoa, string CodInstrumento)
        {
            try
            {
                ...
                objBLL = new BLL_pessoa();
                listaPessoaMetodo = objBLL.buscarMetodoPessoa(CodPessoa);

                objBLL = new BLL_pessoa();
                listaMetodo = objBLL.buscarMetodos(CodPessoa, CodInstrumento);

                foreach (MOD_metodoInstr objEnt_Met in listaMetodo)
                {
                    MOD_pessoaMetodo ent = new MOD_pessoaMetodo();

                    ent.CodPessoaMetodo = "0";
                    ent.CodMetodo = objEnt_Met.CodMetodo;
                    ent.DescMetodo = objEnt_Met.DescMetodo;
                    ent.TipoEscolha = objEnt_Met.TipoEscolha;
                    ent.PaginaFase = objEnt_Met.PaginaFase;
                    ent.Tipo = objEnt_Met.Tipo;
                    ent.Ativo = objEnt_Met.Ativo;
                    ent.Compositor = objEnt_Met.Compositor;
                    ent.CodPessoa = "0";
                    ent.Marcado = false;
        ...
        internal void verPermAtendimento()
        {
            try
            {
                ...
                if (funcoes.liberacoes(entAcesso.rotPesAtendimento))
                {
                    ...
                    if (tabAtendeRegiao.Enabled.Equals(true))
                    {
                        tabAtende.SelectTab("tabAtendeRegiao");
                    }
                    else if (tabAtendeComum.Enabled.Equals(true))
                    {

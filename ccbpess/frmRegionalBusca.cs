...
namespace ccbpess
{
    public partial class frmRegionalBusca : Form
    {
        ...
        private void abrirForm(string form, DataGridView dataGrid)
        {
            try
            {
#warning Criar Formulario Pesquisa Regional.
                if (form.Equals("frmReg"))
                {
                    //formulario = new frmPesquisaRegiao(this, dataGrid);
                ...
                {
                    if (((frmRegional)formulario) == null || ((frmRegional)formulario).IsDisposed)
                    {
                        preencher(Codigo);
        ...
        internal void verPermCodigo(DataGridView dataGrid)
        {
            try
            {
                foreach (MOD_acessos ent in listaAcesso)
                {
                    ...
                    if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotInsRegional))
                    {
                        btnCodIns.Enabled = true;
                    }
                    //verificando o botão editar
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotEditRegional))
                    {
                    ...
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotExcRegional))
                    {
                        if (dataGrid.Rows.Count > 0)
                        {
                    ...
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotVisRegional))
                    {
                        if (dataGrid.Rows.Count > 0)
                        {
            ...
            try
            {
                foreach (MOD_acessos ent in listaAcesso)
                {
                    ...
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotEditRegional))
                    {
                        if (dataGrid.Rows.Count > 0)
                        {
                            btnEstEditar.Enabled = true;
                        }
                        else
                        {
                            btnEstEditar.Enabled = false;
                        }
                    }
                    //verificando o botão excluir
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotExcRegional))
                    {
                        if (dataGrid.Rows.Count > 0)
                        {
                            btnEstExc.Enabled = true;
                        }
                        else
                        {
                            btnEstExc.Enabled = false;
                        }
                    }
                    //verificando o botão visualizar
                    else if (Convert.ToInt32(ent.CodRotina).Equals(modulos.rotVisRegional))
                    {
                        if (dataGrid.Rows.Count > 0)
                        {
                            btnEstVisual.Enabled = true;
                        }
                        else
                        {
                            btnEstVisual.Enabled = false;
                        }
                    }
                }
            }
